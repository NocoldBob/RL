...
这一篇没有把 PPO 描述成一个神奇公式。它真正增加的是一套更完整的策略优化流程：先收集
on-policy 轨迹，用 GAE 估计优势，对同一批数据训练多轮，再用 Clip 限制策略变化。

实验结果同样不完美。PPO 与 DQN 都学到了寻找食物和延长生存的方法，但三个随机种子仍有
明显差异，平均通关率也只有约 15%。这正适合下一步继续追问：问题来自价值高估、策略表示、
奖励设计，还是训练预算？

下一篇可以沿两条路线展开：

1. 在 DQN 上加入 Double DQN 与 Dueling Network，研究价值估计怎样更稳定；
2. 给贪吃蛇增加障碍和课程难度，让算法不只记住固定规模的小地图。
